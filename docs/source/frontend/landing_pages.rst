...
The landing page for unauthenticated users is designed to provide an overview of the app's features and encourage users to sign up 
or log in. 
...
`_HowItWorksSection` is the section of the landing page that explains the platform's core steps, from exploring events, to joining events
and than hosting events. 
...
``landing_page_logged_in.dart`` is the landing page for users who are logged into unisphere. It dynamically presents event discovery 
tools, personalized ticketing information, and organizer metrics depending on the active user's role.
...
Personalized landing page section
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Discover section 
^^^^^^^^^^^^^^^^
The landing page for logged in users features it's own event discover section that users can easily browse events.
